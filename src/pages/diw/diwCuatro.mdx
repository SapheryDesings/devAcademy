...
+ **Pasándole una función** que nos especifique como se comportará. 
    ```javascript
        $("p").click(function() {alert($(this).text());})
    ```
+ **Sin pasarle una función**, por lo que generará un evento estándar de Javascript. No tendremos que usar el prefijo on. 
    ```javascript
      $("p").click()
    ```
